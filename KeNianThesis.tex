...
\ref{sec:Synexperiment}, we will show that spline kernel performs better or equally well when compared with Gaussian kernel.  In the latter discussion, we denote $\delta^{DKL}_{t,T,K}= \delta(\vx_{t}^{T,K}{\valpha}^*)$ as the hedging position form the direct kernel hedging model.
...
\[\pmb{B}\vecVmkt=\{
	...
	\}
\]
where $V(\vx_{t}^{T,K}; \valpha^*)$ is the option value funtion given in equation  \eqref{eq:KernelRep} with the solution $\valpha^*$ given by $\valpha^*=(\vK+\lambda_P \pmb{I})^{-1}\vecVmkt$.
...
For local risk hedging, each data instance is uniquely determined by the triplet $\{t,T,K\}$. We only need the market option quotes of strike $K$ and expiry $T$ to be available on both $t$ and $t+\Delta t$ to gather a data instance for the data-driven local hedging model. Therefore, purely building a model based on market quotes for local risk hedging is achievable. 


For total risk hedging, if we assume the starting hedging date $t_0$ is $N_H$ days to expiry $T$, each data instance is then uniquely determined by the duplet $\{T,K\}$. If we only rely on market quotes to build the total risk model,  we will face the following challenges:
\begin{enumerate}
		...
		\item In addition, the option with specific strike $K$ and expiry $T$ may not be traded or quoted on every trading date during its lifetime. Requiring the market option quotes of strike $K$ and expiry $T$ to be available on entire hedging horizon $[t_0, T]$ is unrealistic especially for in-the-money and out-of-money options. 
		Therefore, we will have to rely on certain parametric models calibrated to market quotes to compute the necessary derived features (e.g., option sensitivity), which is used as the input of the data-driven model, when there are not market quotes or prices for the specific combination of $T$ and $K$ on some trading dates.
...
\begin{enumerate}
\item  \label{item:butterfly}\underline{Butterfly Arbitrage}: At time $t$, given a collection of call option prices $\{C(T,K)\}_{T,K}$, using Dupire's method \cite{dupire1994pricing}, one can write the option value with  an implied probability density $\{ p(\cdot;T,S_t) \}_{T}$ such that 
\[
C(T,K) = D(t,T) \int_{(0,\infty)} (S - K)_+ p(S;T,S_t) dS.
\]
We say that the surface $\{ \sigma^{imp}(T,K) \}_{T,K}$, where $\sigma^{imp}(T,K)$ is the Black-Scholes implied volatility, is free of Butterfly Arbitrage if its implied probability density $\{p(\cdot ; T,S_t)\}_{T}$ is a valid density, i.e., $p(S;T,S_t) \geq 0$ for all $S > 0$ and $\int_{0}^{\infty} p(S;T,S_t) dS = 1$. 
...
\item \label{item:calendar} \underline{Calendar Arbitrage}:
 ...
 \]
 where $\tau =T-t$ and $k$ is parameterized by log-moneyness, i.e. $k := \log(K/F(t;T))$, and $F(t;T)=S_te^{(r-q)(T-t)}$ is the at-the-money forward (ATMF) price for $S_t$. Let $t=T_0 < T_1 < \dots< T_{max} $ be a set of expiries. We say that the surface $\{ \sigma^{imp}(T,K) \}_{T,K}$ is free of Calendar Arbitrage 
\begin{itemize}
...
% \end{equation}
\end{enumerate}
Since they are six  unknown parameters $\{\mu_{L}, \mu_{U},\sigma_{L},\sigma_{U}, \lambda_{L}, \lambda_{U}\}$, additional caliration condition are imposed. Observing that the Black-Scholes \cite{black1973pricing} model implies that, under the risk neutral measurement, the prices of the underlying asset $S_T$ at the maturity $T$ are log-normal distribued:
 \[
 \ln(S_T) \sim  \mathcal{N}(\ln(S_t)+(r-q-\frac{\sigma^2}{2})(T-t),\sigma^2 (T-t))
 \]
we set $\mu_L=\ln(S_t)+(r-q-\frac{\sigma_L^2}{2})(T-t)$ and $\mu_U=\ln(S_t)+(r-q-\frac{\sigma_U^2}{2})(T-t)$ such that we have only four parameters to be solved: $\{\sigma_{L},\sigma_{U}, \lambda_{L}, \lambda_{U}\}$. 
...
\begin{align}
	 ...
	 \label{eq:SABRFix14}
	\end{align}
 we can  show the  intergrability constraint \eqref{eq:RND1} and martingale  constraint \eqref{eq:RND2} are also satisified.  Furthermore, the solution to the equantions \eqref{eq:SABRFix11} to \eqref{eq:SABRFix14} is $\{\sigma_{L}=\sigma_{B}(K_L),\sigma_{U}=\sigma_{B}(K_U), \lambda_{L}=1, \lambda_{U}=1\}$. Therefore, if we have $\{\sigma_{L}=\sigma_{B}(K_L),\sigma_{U}=\sigma_{B}(K_U), \lambda_{L}=1, \lambda_{U}=1\}$, intergrability constraint \eqref{eq:RND1} and martingale  constraint \eqref{eq:RND2} are satisified. 
 
Here, we firstly show the intergrability constraint \eqref{eq:RND1} is satisified if we have equations \eqref{eq:SABRFix11} to  \eqref{eq:SABRFix14} hold. Comnbining equation  \eqref{eq:SABRFix01} and equation \eqref{eq:SABRFix11}, we have:
 \begin{equation}
...
\begin{equation}
	...
	\label{eq:IntProof3}
	\end{equation}
Combining equations \eqref{eq:IntProof1} and \eqref{eq:IntProof2} with equation \eqref{eq:IntProof3}, we show that  the intergrability constraint \eqref{eq:RND1} is satisfied.
...
\begin{equation}
   ...
   \label{eq:RND6}
\end{equation}
We therefore prove the martingale  constraint \eqref{eq:RND2} is satisified.
We will simply using the adjusted p.d.f for option pricing:
\begin{equation}
...
\end{array} \right.
 \]
Here we use the $\overline{C}_{SABR}(T,K)$ to indicate it is the SABR model value after the  fix for butterfly arbitrage. 




In this thesis, we  use the  risk neutral adjustment \eqref{eq:OurFix} with 4 parameters  $\{\sigma_{L},\sigma_{U}, \lambda_{L}, \lambda_{U}\}$ due to its simplicity. Notice that density function $\hat{g}(x)$ calibrated following the above risk neutral adjustment maybe discontinuous at $K_L$ and $K_U$.  If one want to ensure the continuity of the density function,  one can solve the overdetermined system suggested by \citet{brunner2003arbitrage} or solve the following system of equations for  $\{\mu_{L}, \mu_{U},\sigma_{L},\sigma_{U}, \lambda_{L}, \lambda_{U}\}$:
 \begin{equation}
 	\begin{split}
 ...
 \lambda_{U} q(K_{U};\mu_{U},\sigma_{U})&=p(K_{U};T,
 	...
 	\end{split}
 \end{equation}
 Notice that here $\mu_{L}$ and $\mu_{U}$ are parameters to be solved as well. 



For calendar arbitrage, we will shift the price by the the following procedure to remove it. Suppose we have a grid of $K_{grid}=\{K_0,K_1,\dots,K_{max}\}$ we have the a grid of expiries $t=T_0 < T_1 < \dots< T_{max}$. The for each expiry  $T_j,i=1,\dots,max$
\[
shift_{T_j}=-min(min_{K \in K_{grid}}(\overline{C}_{SABR}(T_j,K)-\overline{C}_{SABR}(T_{j-1},K)),0).	
\]
\[
...
\]
 We comments that call-spread arbitrage violation is rare and no violations of this condition from the prices produced by SABR model  are observed for all the computational results in this thesis. 
 \subsubsection{Alternative Methods to SABR Calibration}
 Given BS implied volatility surface market data as a discrete set of points $\{ \sigma^{imp}_{BS,mkt}(T,K) \}_{T,K}$, a natural question is whether static arbitrage exists. In most cases, one must 
 \begin{enumerate}
 \item Fit a parameterization to the points $\{ \sigma^{imp}_{BS,mkt}(T,K) \}_{T,K}$, typically by time-slice as what we did with SABR model, obtaining a parameterization $\widetilde{\sigma}^{imp}_{model}(T_i,K)$ for each $T_i$ separately.
 For example, if we fit a SABR model, $\widetilde{\sigma}^{imp}_{model}(T_i,K)=\sigma_{B}(F,t,T_i,K;\alpha,\beta,\nu,\rho)$ and since except $K$ all other parameters are fixed for a expiry $T_j$ on a time $t$, it is a function of $K$ only.
...
\end{equation}
where 
$I$ is the identity matrix, $\mathcal{\mathbb{S}}$ is a diagonal matrix parameterized by $\widehat{\sigma}(T_i,.)$ as in equation \eqref{eq:LVFVolDef}, and $\mathcal{\mathbb{D}}$ is proportional to the discrete second order difference matrix. Specifically:
		\begin{equation}
...
\begin{bmatrix}
	...
	\end{bmatrix}.
\]
We want  the price vector at $T_{i+1}$ produced by the above LVF to match the price vector we computed using SABR model on $T_{i+1}$. In other words, we will try to find the $\widehat{\sigma}(T_i,.)$, which has the form as in equation \eqref{eq:LVFVolDef}, by solving the following non-linear least square problem:
\begin{equation}
...
\] $Vega_{B}(T_{i+1},\widehat{K}_j)$ is the vega computed using SABR model calibrated to market quotes at $T_{i+1}$ and $\widetilde {C_{SABR}}(T,K)$ is the arbitrage-free SABR model value we produce in previous section.
...
Note that for the intial case $T_0=t$, $\widehat{C}(T_0,\widehat{K})=max(1-\widehat{K},0)$ is given as the payoff. Given $\widehat{C}(T_0,\widehat{K})$, we firstly solve \eqref{eq:LVFCal} for the $\widehat{\sigma}(T_0,.).$ After obtaining $\widehat{\sigma}(T_0,.)$, we can then solve the forward system  $\eqref{eq:LVFCal}$ to get \[
	\begin{bmatrix}
		...
		\end{bmatrix}
\] sequentially for $i=1,2,\dots,T_{max-1}$




After we solved for  $\widehat{\sigma}(T_i,.), i=0,1,\dots,T_{max-1}$, for $T \in [T_i,T_{i+1}]$, we can fill in the gaps by:
	\begin{equation}
	\begin{bmatrix}
	...
	\end{bmatrix}=\mathcal{\mathbb{M}}^{-1} (T,T_{i}, \widehat{\sigma}(T_i,.))\begin{bmatrix}
		...
		\label{eq:LVFFill}
	\end{equation}
where $\mathcal{\mathbb{M}} (T,T_{i}, \widehat{\sigma}(T_i,.))=I-\mathcal{\mathbb{S}}\mathcal{\mathbb{D}}(T-T_i)$ is a tri-diagonal matrix parametrized by  $\sigma(T_i,.)$, $T$ and $T_i$. Note that $\sigma(T_i,.)$ is known after the calibration \eqref{eq:LVFCal}.
...
\begin{thm}
	On the discrete strike grid $\widehat{K}_0,\dots, \widehat{K}_{max}$, the finite difference scheme \eqref{eq:LVFInt} and  \eqref{eq:LVFFill} produces call option prices that are convex in strikes:
	% \[
...
\begin{proof}
	...
	\\\textbf{Base case:}
	For the first expiry $T_0=t$, we have $\widehat{C}(T_0,\widehat{K})=max(1-\widehat{K},0)$ which is the normalized payoff. We naturally have the convexity in strikes at $T_0$. Furthermore, this also implies all elements in $\mathcal{\mathbb{D}}\widehat{C}(T_0)$ are non-negative.
	\\\textbf{Induction step:}
	Assume we have the convexity in strikes at $T_i, i=0,1,\dots,max-1$, i.e., all elements in $\mathcal{\mathbb{D}}\widehat{C}(T_i)$ are non-negative.
	The finite difference scheme \eqref{eq:LVFFill} can be written as the matrix equation system 
	\begin{equation}
	...
	\begin{equation}
	[\mathcal{\mathbb{S}}^{-1}-\widetilde{\mathcal{\mathbb{D}}}](\mathcal{\mathbb{S}}\widetilde{\mathcal{\mathbb{D}}} \widehat{C}(T))=\widetilde{\mathcal{\mathbb{D}}} \widehat{C}(T_i)
	\end{equation}
	...
	% 	\[u_i=\frac{1}{\widehat{K}_{i+1}-\widehat{K}_{i-1}}\frac{1}{\widehat{K}_{i+1}-\widehat{K}_{i}}.\]
		...
		if elements in the vector $\mathcal{\mathbb{D}}\widehat{C}(T_i)$ are all non-negative, then elements in  $\widetilde{\mathcal{\mathbb{D}}}\widehat{C}(T_i)$ are all non-negative.
		With all elements of $\mathcal{\mathbb{A}}^{-1}$ being non-negative, we can easily show that elements in the vector $\mathcal{\mathbb{S}}\widetilde{\mathcal{\mathbb{D}}}\widehat{C}(T)$ are all non-negative. Furthermore, since:
		\[
			\mathcal{\mathbb{S}}\widetilde{\mathcal{\mathbb{D}}}\widehat{C}(T)	=	(T-T_i)\mathcal{\mathbb{S}}\mathcal{\mathbb{D}} \widehat{C}(T)
		\]
		Thus all elements in $\mathcal{\mathbb{D}}\widehat{C}(T)$ are non-negative which is equivalent to show that:
		\[
...
\end{proof}
\begin{thm}
	On the discrete strike grid $\widehat{K}_0,\dots, \widehat{K}_{Max}$, the finite difference scheme \eqref{eq:LVFInt} and  \eqref{eq:LVFFill} produces call option prices increasing in maturity:
	\[
			C(T_i,K_{i})\geq C(T_{i-1},K_{i}) \; \text{ when } T_{i}>T_{i-1}
	\]
...
\end{thm}
\begin{proof}
	Recall that, for $T \in (T_i,T_{i+1}], i=0,\dots, max-1$, we rewrite the finite difference scheme \eqref{eq:LVFFill}  as:
	\begin{equation}
		[I-\mathcal{\mathbb{S}}\mathcal{\mathbb{D}}(T-T_i) ]\widehat{C}(T)=	\widehat{C}(T_i)
		\label{eq:LVFProof3}
	\end{equation}
Differentiating equation \eqref{eq:LVFProof3} by $T$ on both side:
\[
	\frac{\partial  [I-\mathcal{\mathbb{S}}\mathcal{\mathbb{D}}(T-T_i) ]}{ \partial T} \widehat{C}(T) +[I-\mathcal{\mathbb{S}}\mathcal{\mathbb{D}}(T-T_i) ]\frac{\partial \widehat{C}(T)}{ \partial T} =  0
\]
Therefore:
\[
	[I-\mathcal{\mathbb{S}}\mathcal{\mathbb{D}}(T-T_i) ]\frac{\partial \widehat{C}(T)}{ \partial T} =  	 \mathcal{\mathbb{S}}\mathcal{\mathbb{D}} \widehat{C}(T)
\]
Multiply $S^{-1}$ on both side:
\begin{equation}
	[S^{-1}-\mathcal{\mathbb{D}}(T-T_i)]\frac{\partial \widehat{C}(T)}{ \partial T} =  	 \mathcal{\mathbb{D}} \widehat{C}(T)
	\label{eq:LVFProof4}
\end{equation}
Recall we define:
\[\mathcal{\mathbb{A}}=\mathcal{\mathbb{S}}^{-1}-\widetilde{\mathcal{\mathbb{D}}}\]
...
Therefore, we can rewrite equation \eqref{eq:LVFProof4} as:
		\begin{equation}
			\frac{\partial \widehat{C}(T)}{ \partial T} =\mathcal{\mathbb{A}}^{-1}   \mathcal{\mathbb{D}} \widehat{C}(T)
			\label{eq:LVFProof5}
		\end{equation}
		Since we already prove the convexity in strikes which implies elements in $\mathcal{\mathbb{D}} \widehat{C}(T)$ are all non-negative and all elements in $\mathcal{\mathbb{A}}^{-1}$ are non-negative since $\mathcal{\mathbb{A}}$ is an M-Matrix \cite{plemmons1977m}, we can further conclude that all elements in  $\frac{\partial \widehat{C}(T)}{ \partial T}$ are non-negative. Therefore we prove the generated option prices are increasing in maturity which proves the absence of calendar arbitrage.
\end{proof}
...
A natural question the reader may ask is that why donot we apply above algorithm with purely market quotes? In other words, we solve below problem instead of problem \eqref{eq:LVFCal}:
\[
...
\]
In reality, it is hard to find a grid of normalized strike: $0=\widehat{K}_0<\widehat{K}_1<\dots<\widehat{K}_{max}$, for which, $\widehat{C}_{mkt}(T_{i},\widehat{K}_j)$,$i=0,1,\dots,max$ all exists,
especially, if we want our grid of strike to cover both in-the-money option and out-of-the-money option. That is why we use SABR model  which can produce option value for any $K$ for a expiry $T_i$ in market. In this way, we can use any grid of strikes as we want.
...
\begin{enumerate}
		\item We pick a training expiry date $\widehat{T}$. Note that  $\widehat{T}$ does not have to be a real expiry that exists in market.  In this way, we greatly increase the number of training scenarios.  
		\item The starting date $t_0$ to set up the initial hedging portfolio is  $N_H$-business days away from $\widehat{T}$. That is to say $\widehat{T}=t_0+N_H/250$ where we assume there are 250 business days in one year. In this thesis,  $N_H$ is set to be 100. We further comment that, in this thesis, on each date  $t \in [t_0, \widehat{T})$, we always have $\widehat{T}<T_{t,max}^{mkt}$, where $T_{t,max}^{mkt}$ is the maximum expiry listed in exchange at time $t$. In other words, no extrapolation is needed.
...
\begin{enumerate}
	...
	When there is no  market quotes $V^{mkt}_{t,T^{mkt},K}$ on time $t \in [t _0, T^{mkt})$ for $K \in \mathbf{K}^{mkt}_{grid}(t _0,T^{mkt})$, we will query $\{V_{t}(T,K)\}_{T,K}$ to obtain  option prices and option sensitivities and we use them to construct a time series. When market data $V^{mkt}_{t,T^{mkt},K}$ and associated option sensitivities  are available, we use the market data to construct the time series needed as the input for the data-driven model. Note that, with this approach, for instance, a part  of the time series of option prices for a testing scenario can be real market quotes while the other part can be model prices from the  parametrization  obtained following the calibration process in section \ref{sec:NoArb}.
	\item The time series of daily underlying prices $S_t, t \in[t_0,T)$ are obtained directly from market. 
...
\begin{itemize}
	...
	\item  For testing scenarios, we use real option market data to construct the timeseries whenever asscoiated  option market data is available. Otherwise, we will query the calibrated option value function.
	\item For training scenarios, we use model prices to initialize the hedging portfolio. The strikes and expiries do not necessarily have to be are real market strikes and expiries.  \footnote{Note that after calibration, for the option with a market quote avaliable in exchange, the model value will be very close to available market quote.}
...
\begin{itemize}
	...
	\begin{itemize}
	\item For testing scenarios, $V_{t,T,K}=\Vmkt_{t,T,K}$ is the market price. 
	\item For training scenarios, $V_{t,T,K}=V^{model}_{t,T,K}$ is the model value.
	\end{itemize}	
...
Assume that we are given   a set  of hedging scenarios identified by the starting date $t^i_0$, expiry date $T_i$ and strike $K_i$:
\[
...
\]
where $\text{Risk}^{total}_{t_0,T,K}$ is defined as in equation \eqref{eq:TotalRiskCh6} and $M$ is the number of hedging scenarios .
...
\begin{equation}
Obj_{total}=\sum_{i=1}^M \left|\frac{D(t_0^i,T_i)  \text{Risk}^{total}_{t_0^i,T_i,K_i}}{V_{t^i_0,T_i,K_i}}\right|
...
We  initialize the  GRU parameters using the same procedure as in section \ref{sec:init} and we pretain the $\modelT$ and $\modelL$ similarly as in section \ref{sec:preT}. Early stopping is used as the regularization techniques. We reserve the validation set to determine when to stop the training. 
We train $\modelT$ and $\modelL$ until trust region algorithms (i.e., Algorithm \ref{algorithm} ) stops and based on either the the total risk objective \eqref{eq:totalObjLinear} or local risk objective \eqref{eq:LocalObjNew}. The  parameters for trust region algorithm is in Table \ref{para2}. Early stopping is used as the regularization, which is the same as in chapter \ref{sec:RNNLocal}.
...
\begin{steps}
	\item For a  testing expiry date $T^{mkt}$, construct the testing set below \footnote{ If we fixed the gap between two rebalancing time to be $\DT$ and we fix the number of times we rebalance the hedging portfolio to be $N_{rb}$, then given a expiry $T$, one can easily deduct the rebalance time $\{t_0,t_1, \dots, t_{N_{rb}-1}\}$ . Therefore, we can uniquely define a hedging scenario just by $T$ and $K$}:
	\[
	TestSet=\{(T,K)|,T=T^{mkt},K \in \mathbf{K}^{mkt}(t_0,T^{mkt})\}
	\] where $\mathbf{K}^{mkt}(t_0,T^{mkt})$ is the grid of market strikes for expiry $T^{mkt}$ that can be observed directly from market on the initial date $t_0$ which is 100 business away from $T^{mkt}$. In other word, the total hedging horizon is $N_H=100$ business days.
...
\end{steps}
Notice that in setp 3, we have included new data into the training and validation sets since the range for the allowed expiries in the training set is extended from $T_{min} \leq T<t_{j-1}$ to be $T_{min} \leq T<t_{j}$ where $t_{j}>t_{j-1}$.
...
\]
Namely, we test on all the market observed combinations of $(T,K)$ where the expiry $T$ is between 2000-0-01 to 2015-08-31. Note that, we have included two crisis periods: the burst of dot-com bubble  period (2000 to 2002)and subprime mortgage crisis period (2007 to 2008). We assume we are on the sell-side of the option trading. 
...
\begin{itemize}
	...
	\item $\modelL$: the model shown in Figure \ref{fig:RNNModelTotal} and is trained with the local risk objective \eqref{eq:LocalObjNew},
	\item Bartlett: Barlett corrective  delta based on \eqref{eq:bartlett},
