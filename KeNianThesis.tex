...
\begin{equation}
    ...
    \label{eq:TotalObjV2}
    \end{equation}
By comparing equation \eqref{eq:local} and \eqref{eq:TotalObjV2}, we can see that the {\em discrete total hedging risk} is summation of the {\em discrete local hedging risk} evaluated at discrete rebalancing time $\{t_0,t_1,\dots,t_{N-1}\}$. 
...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\item In addition, option with specific strike $K$ and expiry $T$ is not traded on every data. Even we use market available expiration dates, on the initial date when we want to set up the hedging portfolio, we may not have the initial market option price $\Vmkt_{t_0,T,K}$.
	\end{enumerate}
\end{enumerate}
...
\[
	\sigma(T_i,K)=\left\{ \begin{array}{ll}
		...
		\vdots & \vdots\\
		\sigma_{T_i,K_Max}  , \; &\text{if} \; K_{max} \geq K\\
		\end{array} \right.
\]
We further assume:
\[
...
\section{Overview of the Discrete Total Hedging Model}
\label{sec:RNNTotal}
\subsection{Synthetic Exploration}

\section{Data-Driven Total Discrete Hedging Model and the Different Choices of Features}
...
\begin{table}[htp!]
	...
	\begin{tabular}{|c|c|c|c|c|c|}
	  ...
	  Total       &0.2255   &0.3243   &0.6838    &0.8266   &1.1632 \\
	  \hline
	\end{tabular}
	\caption{Total Risk Weekly}
\end{table}
