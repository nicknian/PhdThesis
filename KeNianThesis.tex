...
Following \cite{gatheral2014arbitrage}, we say that an implied volatility surface is free of \textit{static arbitrage} if and only if it is free of \textit{call spread arbitrage}, \textit{calendar arbitrage}, and \textit{butterfly arbitrage}, which we describe below. We assume we have a collection of European call option prices $\{C(T,K)\}_{K,T}$ for a range of strikes, $K$, and maturities, $T$, from which we associate its Black-Scholes implied volatility surface $\{ \sigma^{imp}(T,K)\}_{K,T}$. We also suppose that interest rates are deterministic with the expressions $D(t;T)=e^{-r(T-t)}$ for discount factors, $t$ be trading date and $T$ be the maturity date.


\begin{enumerate}
\item  \label{item:butterfly}\underline{Butterfly Arbitrage}: Given a collection of call option prices $\{C(T,K)\}_{K,T}$, using Dupire's method or equivalent approaches \cite{dupire1994pricing} one can (at least formally) find expressions for implied probability densities $\{ p(\cdot;T,S_0) \}_{T}$ such that 
\[
C(T,K) = D(t;T) \int_{(0,\infty)} (S - K)_+ p(S;T,S_t) dS.
\]
We say that the surface $\{ \sigma(T,K) \}_{K,T}$, where $\sigma(T,K)$ is the black-scholes implied volatility, is free of Butterfly Arbitrage if its implied probability densities $\{p(\cdot ; T,S_0)\}_{T}$ are valid densities, i.e., $p(S;T,S_t) \geq 0$ for all $S > 0$ and $\int_{0}^{\infty} p(S;T,S_t) dS = 1$. 
...
While the above definition is a proper theoretical starting point for working with butterfly arbitrage, one can also characterize the presence of butterfly arbitrage in terms of the observed volatility surface. Given a surface $\sigma(T,K)$, we consider the corresponding total variance (TV) surface defined by 
 \[
 w(k,T) = \sigma(k,T)^2 T
 \]
 where now $k$ is parameterized by log-moneyness, i.e. $k := \log(K/F(t;T))$ and $F(t;T)=S_te^{(r-q)(T-t)}\\$ is the forward price for $S_t$: . 
We say that the surface $\{ \sigma(K_i,T_j) \}_{K,T}$ is free of Calendar Arbitrage 
\begin{itemize}
\item if $\frac{\partial w(k,T)}{\partial T} \geq 0 $ for all $k \in \mathbb{R}, T > 0$ for continuous time data
...
Since in this subsection, we are discussing how to obtain a arbitrage-free surface on a fixed date $t$, for the notational simplicity, we will omit the superscript  and subscript $t$ in this subsection \ref{sec:NoArb}, we denote the grid of market observed expires on t to be \[
\{ T_{0}<
T_1 < \dots< T_{max}
\}
\]
...
\end{array} \right.
\]
where  $q(x;\mu,\sigma)$ is the p.d.f of a  log-normal distribution with volatility:
\[
	q(x;\mu,\sigma)=\frac{1}{x\sigma \sqrt{2 \pi}}
	e^{-\frac{(\log(x)-\mu)^2}{2 \sigma^2}}.
\]
and $p(x;T,S_t)$ is the implied probability density from calibrated SABR model.
...
We observe that:
\begin{equation}
\int_{K_{L}}^{K_{U}}	  p(x;T,S_t)  dx =1-DP(T,K_{L};\sigma_{B}(K_{L}))-DC(T,K_{U};\sigma_{B}(K_{U}))
\end{equation}
\begin{equation} \begin{split}
\int_{K_{L}}^{K_{U}}	 x p(x;T,S_t)  dx &=
F(t,T)-[K_{L}DP(T,K_{L};\sigma_{B}(K_{L}))-P(T,K_{L};\sigma_{B}(K_{L}))]\\
&+[K_{U}DC(T,K_{U};\sigma_{B}(K_{U}))+C(T,K_{U};\sigma_{B}(K_{U}))]
\end{split}
\end{equation}
where:
\begin{itemize}
\item  We let $C(T,K;\sigma_B)$ and $P(T,K;\sigma_B)$ be the call and put option prices from Black's model with the Black's implied volatility $\sigma_B$.
\item  We let $DC(T,K;\sigma_B)$ and $DP(T,K;\sigma_B)$ be the digital call and put option prices from Black's model with the Black's implied volatility $\sigma_B$  where a digital call pays one dollar if the underlying price exceeds the strike and a digital put pays the same amount  if the underlying is below the strike. 
\item We have $\sigma_B$ given by the SABR apprximation formula \eqref{eq:SABRExpansion} for $K \in [K_{L},K_{U}]$. For simplicity, we wrote $\sigma_{B}(K)$ to denote the implied black's volatility given by the SABR formula \eqref{eq:SABRExpansion} for strike $K$.
 \end{itemize}
Therefore, if we have:
\[
\int_{0}^{K_{L}}	\lambda_{L}   q(x;\mu_{L},\sigma_{L}) dx=DP(T,K_{L};\sigma_{B}(K_{L}))
\]
\[
\int_{K_{U}}^{\infty}	 \lambda_{U} \;  q(x;\mu_{U},\sigma_{U}) dx=DC(T,K_{U};\sigma_{B}(K_{U}))
\]
\[
\int_{0}^{K_{L}} (K_L-x)	\lambda_{L}   q(x;\mu_{L},\sigma_{L}) dx=P(T,K_{L};\sigma_{B}(K_{L}))
\]
\[
\int_{K_{U}}^{\infty}	(x-K_U) \lambda_{U} \;  q(x;\mu_{U},\sigma_{U}) dx=C(T,K_{U};\sigma_{B}(K_{U}))
\]
we can easily show the  intergrability constraint \eqref{eq:RND1} and martingale  constraint \eqref{eq:RND2} are satisified. As a result, we obtain the following system of equations, which we can solve for  $\{\mu_{L}, \mu_{U},\sigma_{L},\sigma_{U}, \lambda_{L}, \lambda_{U}\}$:
\begin{equation}
	\begin{split}
\int_{0}^{K_{L}}	\lambda_{L}   q(x;\mu_{L},\sigma_{L}) dx&=DP(T,K_{L};\sigma_{B}(K_{L})\\
\int_{K_{U}}^{\infty}	 \lambda_{U} \;  q(x;\mu_{U},\sigma_{U}) dx&=DC(T,K_{U};\sigma_{B}(K_{U})\\
...
Furthermore, recall that the Black-Scholes-Merton \cite{black1973pricing} model implies that the prices of the underlying asset $S_T$ at the maturity $T$ are log-normal distribued:
\[
\ln(S_T) \sim  \mathcal{N}(\ln(S_t)+(r-q-\frac{\sigma^2}{2})(T-t),\sigma^2 (T-t))
\]
Therefore, we can set $\mu_L=\ln(S_t)+(r-q-\frac{\sigma_L^2}{2})(T-t)$ and $\mu_U=\ln(S_t)+(r-q-\frac{\sigma_U^2}{2})(T-t)$. In this way, we only need to solve for  $\{\sigma_{L},\sigma_{U}, \lambda_{L}, \lambda_{U}\}$ after we set $\mu_L=\ln(S_t)+(r-q-\frac{\sigma_L^2}{2})(T-t)$ and $\mu_U=\ln(S_t)+(r-q-\frac{\sigma_U^2}{2})(T-t)$. 
As we can easily see, Equation \eqref{eq:SABRFix1} can be further simplified as:
\begin{equation}
	\begin{split}
		\lambda_L DP(T,K_{L};\sigma_L)&=DP(T,K_{L};\sigma_{B}(K_{L}))\\
		\lambda_U DC(T,K_{L};\sigma_U)&=DC(T,K_{U};\sigma_{B}(K_{U}))\\
		\lambda_L P(T,K_{L};\sigma_L)&=P(T,K_{L};\sigma_{B}(K_{L}))\\
		\lambda_U C(T,K_{L};\sigma_U)&=C(T,K_{U};\sigma_{B}(K_{U}))\\
% \lambda_{L} q(K_{L};\mu_{L},\sigma_{L})&=p(K_{L};T,S_t)\\
% \lambda_{U} q(K_{U};\mu_{U},\sigma_{U})&=p(K_{U};T,
% 	S_t)
	\end{split}
...
\[
	C(T,K)=\int_{-\infty}^{\infty} (x-K)_+ \hat{g}(x) dx
\]
\[\small
	C(T,K)=\left\{ \begin{array}{ll }
...
\[\begin{split}
	...
	\\
	&=\lambda_L \left\{C(T,K;\sigma_L)-\left[C(T,K_L;\sigma_L)+(K_L-K)DC(T,K_L;\sigma_L)\right] \right\}
\end{split}
\]
Similarly,
\[\begin{split}
	\int_{K_L}^{K_U} (x-K) p(x;T,S_t)dx&=\left[C(T,K_L;\sigma_B(K_L))+(K_L-K)DC(T,K_L;;\sigma_B(K_L))\right]-\left[C(T,K_U;\sigma_B(K_U))+(K_L-K)DC(T,K_L;\sigma_B(K_U))\right]
\end{split}
\]
\[\begin{split}
	...
	&=\lambda_U \left[C(T,K_U;\sigma_U)+(K_U-K)DC(T,K_U;\sigma_U)\right]
\end{split}
\]
\[\begin{split}
	\int_{K}^{\infty} (x-K) \lambda_U q(x;\mu_{U},\sigma_{U}) dx&=\lambda_U C(T,K;\sigma_U)
\end{split}
\]
\[\begin{split}
...
\[\small
	C(T,K)=\left\{ \begin{array}{ll }
		\lambda_L C(T,K;\sigma_L)\;&  \text{if} \; 0<K< K_{L}\\

		C(T,K;\sigma_B(K))\;&  \text{if} \; \;  K_{L} \leq K \leq K_{U}\\

		\lambda_U C(T,K;\sigma_U) \; &  \text{if} \; \;  K> K_{U}
\end{array} \right.
\]
...
\[
shift=-min(min(C(T_j,K)-C(T_{j-1},K)),0).	
\]
\[
	\widehat{C}(T_j,K)=C(T_j,K)+shift
\]
 Note the shift will be zero if no calendar arbitrage is observed for the grid of strikes.

\subsubsection{Volatility Interpolation Between Expiries}
In previous section, for each $T_i$, we calibrate a separate set of SABR parameters and we then use the calibrated SABR parameters to compute SABR price of the predetermined grid of strikes for each $T_i$.  The SABR prices of the predetermined grid of strikes are used as the calibration targets for the volatility interpolation procedure described in the this subsection.
