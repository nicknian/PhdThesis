...
Furthermore,  given a  grid of strikes: $0=K_0<K_1<\dots<K_{N}$, and a grid of expiries $t=T_0 < T_1 < \dots< T_{M} $.   The corresponding discrete criteria \cite{carr2005note} for a grid of  option prices to be free of arbitrage are set as the following: 
\begin{enumerate}
...
For calendar arbitrage, we will shift the price by the the following procedure to remove it. Suppose we have a grid of $\mathbf{K}_{grid}=\{K_0,K_1,\dots,K_{N}\}$ and  a grid of expiries $t=T_0 < T_1 < \dots< T_{M}$. Then for each expiry  $T_j,i=1,\dots,M$
\[
...
\]
 ...
 Given BS implied volatility surface market data as a discrete set of points $\{ \sigma^{imp}_{BS,mkt}(T,K) \}_{T,K}$, a natural question is whether  arbitrage exists. In most cases, one must 
 \begin{enumerate}
...
\begin{itemize}
	\item $\{S_t|\forall t \in \mathbf{t}_B \}$: the time-series of underlying prices  with $t_0$ as the initial date and $T$ as the expiry date.
	\item $\{V_{t,T,K}|\forall t \in \mathbf{t}_B\}$:  time-series of option value for a hedging scenario identified by $(T,K)$.
...
\begin{algorithm}[htp!]
	...
	\Fn{\FMain{$t$, $T$, $\mathbf{K}_{grid}$}}{
		...
		Check if there is any violation of the following butterfly arbitrage on the grid of strikes $\mathbf{K}_{grid}$ with $i=1,\dots, N-1$:
		\begin{equation}
	...
	%		  b\;
	%	}
	\caption{SABR Calibration Function} 
...
\begin{algorithm}[htp!]
	...
	\SetKwProg{Fn}{Function}{:}{}
	\Fn{\FMain{ $t$,$\mathbf{T}_{mkt}$,, $\mathbf{K}_{grid}$,$\mathbf{T}_{grid}$ }}{
		\KwIn{ $t$: An option trading date $t$.\newline
			$\mathbf{T}^{mkt}_t=\{T_0=t, \dots, T_M\}$: A grid of \underline{\textbf{market}} available expiries at time $t$.\newline
			$\mathbf{K}_{grid}=\{K_0, K_1, \dots, K_N\}$: A grid of strikes for outputting option value.\newline
...
\begin{algorithm}[htp!]
	...
	\Fn{\FMain{$T$,optType}}{
		\KwIn{ 
			   $T$: A expiry date for the hedging scenarios.\newline
			   optType: Call or put option. \newline
...
\Fn{\FMain{$T$,optType}}{
	...
	Set	$t_0=T-\frac{100}{250}$: A initial date for setting up the hedging scenarios.\;
	Extract $\mathbf{t}_B=\{t_0,\dots,t_N=T\}$: the set of business dates between $t_0$ and $T$ sorted in ascending order.\;
		Extract all market available strikes at $t_0$ for the expiry $T$ as the grid of strikes:
	...
	\caption{Function For Constructing Testing Hedging Scenarios With Initial Date $t_0$ and Expiry Date $T$ } 
...
\begin{steps}
	...
	\item For a testing expiry date $T^{mkt} \in \mathbf{T}_{AllTest}$, we construct the testing set below:
	\[
	TestSet=\{Scenario(T^{mkt},K)|\forall K \in \mathbf{K}^{mkt}_{grid}(t_0,T^{mkt})\}
	\] where $\mathbf{K}^{mkt}_{grid}(t_0,T^{mkt})$ is the grid of market strikes for expiry $T^{mkt}$ that can be observed directly from market on the initial date $t_0$. And $t_0$ is 100 business away from $T^{mkt}$. In other word, the total hedging horizon is $N_H=100$ business days. We build a sequence of models to hedge those testing scenarios with the same expiry date $T^{mkt}$ and different strikes $K \in \mathbf{K}^{mkt}_{grid}(t_0,T^{mkt})$. The testing scenario: $Scenario(T^{mkt},K)$ is constructed using procedure described in Algorithm \ref{alg:TestConstruction}.
	...
	In other words, we include all training hedging scenarios for which the expiry dates are before $t_0$.
	
	The validation set is:
	\[ValSet=\{Scenario(T,K)|T=t_0,  \forall K \in \mathbf{K}_{grid}(T-\frac{100}{250},T)\}\]
	In other words, we include all hedging scenarios for which the expiry date is $t_0$ as the validation set.
...
\begin{algorithm}[htp!]
	...
	\SetKwProg{Fn}{Function}{:}{}
	\Fn{\FMain{ $T^{mkt}$, optType}}{
			\KwIn{ 
				$T^{mkt}$: A \underline{\textbf{market}} expiry date for the testing hedging scenarios.\newline
				optType: Call or put option. \newline
			...
			Set  $t_0 \leftarrow T^{mkt}-\frac{100}{250}$\;
			Extract $\mathbf{t}_{RB}=\{t_0,\dots,t_j,\dots, t_{N_{rb}-1}\}$: the set of \underline{\textbf{rebalancing}} dates sorted in ascending order. Each rebalancing time is $t_j=t_0+j\times \Delta t$ and $\Delta t$ is the gap between the adjacent rebalancing date.\;
			Extract all market available strikes at $t_0$ for the expiry $T^{mkt}$ as the grid of strikes:$\mathbf{K}^{mkt}_{grid}(t _0,T^{mkt})$\;
			TestSet $\leftarrow \texttt{TestingScenarioGeneration}(T^{mkt}, \text{optType})$\;
			Set $T_{min} \leftarrow t_0-3$\;
			...
			Bulid model $\modelT$ based on TrainSet\;
			Validate model $\modelT$ based on ValSet\;
			Compute the hedging position at $t_j$ for TestSet: $\{\delta^M_{t_j,T,K}|\forall K \in \mathbf{K}^{mkt}_{grid}(t_0,T), T=T^{mkt}\}$\;
			}
			\KwRet $\{\delta^M_{t,T,K}|\forall t \in \mathbf{t}_{RB},\forall  K \in  \mathbf{K}^{mkt}_{grid}(t_0,{T}),T=T^{mkt} \}$			
		}
	
	\caption{Function For Building Model To Hedge Scenarios With the Expiry Date $T^{mkt}$} 
...
We  initialize the  GRU parameters using the same procedure as in section \ref{sec:init} and we pretain the $\modelT$ similarly as in section \ref{sec:preT}. Early stopping is used as the regularization techniques. We reserve the validation set to determine when to stop the training. 
We train $\modelT$until trust region algorithms (i.e., Algorithm \ref{algorithm} ) stops and based on either the the total risk objective \eqref{eq:totalObjLinear}. The  parameters for trust region algorithm is in Table \ref{para2}. Early stopping is used as the regularization, which is the same as in chapter \ref{sec:RNNLocal}. The overall model building procedure is given in Algorithm \ref{alg:ModelBuilding}.
...
The sequential inputs to $\modelT$, $\mathbf{Y}_{t}^{T,K}$, at a rebalancing time $t$, at each hedging time $t$, are the historical time series recorded daily from previous rebalancing time $t-\DT$ to current rebalancing time $t$ for the following features:
...
Additionally,  since $\model$ is built on top of pure market data while $\modelT$ is built on top of augmented market data, we will not compare the performance of  $\model$ and $\modelT$ directly. Since, here we are more interested in the effect of the choice of objective functions. Therefore, we define a new comparing model $\modelL$. The model structure of $\modelL$ is exactly the same as  $\modelT$ which is as in Figure \ref{fig:RNNModelTotal}.  The only difference is $\modelT$ is built on minimizing objective \eqref{eq:totalObjLinear}. While the $\modelL$ is built on minimizing a local risk objective. The detailed comparison between $\modelT$ and $\modelL$ is given in Appendix \ref{app:CompareLocalVSTotal}.
...
\begin{table}[htp!]
	...
	\begin{tabular}{ll|l|l|l|}
		...
		\multicolumn{1}{|l|}{\multirow{3}{*}{CVaR (99\%)}}    & $\modelT$    & \textbf{-0.8205}          & \textbf{-0.3261} & -1.6090          \\  
...
mean absolute relative hedging error for OTM scenarios. In terms of VaR and CVaR, by comparing Table \ref{table:CallTotalM} and Table \ref{table:CallTotalW}, $\modelT$ is less dominant  in monthly hedging call options than in weekly hedging call options.  Bartlett delta produces best VaR(99\%) and CVaR(99\%) for NTM scenarios. However, from Table \ref{table:CallTotalM} we can also see, the performance of $\modelT$ is usually very close to best performance even if  $\modelT$ is not the dominant model in terms of certain criteria.
\begin{table}[htp!]
	\centering
	\begin{tabular}{ll|l|l|l|}
		\cline{3-5}
...
\subsubsection{Call Option Weekly Hedging Comparison}
In Table \ref{table:AppCallTotalW}, we demonstrate the results on weekly hedging call options. Furthermore, in Figure \ref{fig:AppCallTotalW1}, we compare the distribution of the relative hedging error of $\modelT$ with the distributions of the relative hedging error of $\modelL$, BS model, and Bartlett model respectively.
\begin{table}[htp!]
	\centering
	\begin{tabular}{ll|l|l|l|}
		\cline{3-5}
		&          & Near-The-Money   & In-The-Money     & Out-of-The-Money \\ \hline
		\multicolumn{1}{|l|}{\multirow{2}{*}{Mean Abs Relative Error}} & $\modelT$    & \textbf{0.1927}  & 0.0571  & \textbf{0.7344}  \\  
		...
		\multicolumn{1}{|l|}{}                                & $\modelL$    & -0.6361          & -0.2168          & -1.3583          \\  \hline 
		\multicolumn{1}{|l|}{\multirow{2}{*}{CVaR (99\%)}}    & $\modelT$    & -0.8205          & -0.3261 & -1.6090          \\  
...
\subsubsection{Call Option Monthly Hedging Comparison}
In Table \ref{table:AppCallTotalM}, we demonstrate the results on monthly hedging call options. Furthermore, in Figure \ref{fig:AppCallTotalM1}, we compare the distribution of the relative hedging error of $\modelT$ with the distributions of the relative hedging error of $\modelL$, BS model, and Bartlett model respectively.

From Table \ref{table:AppCallTotalM}, we can see that, $\modelT$ performs better than the other three methods in terms of 
mean absolute relative hedging error for NTM and ITM scenarios. Bartlett method performs best in terms of 
...
\end{itemize}
\subsubsection{Put Option Weekly Hedging Comparison}
In Table \ref{table:AppputTotalW}, we demonstrate the results on monthly hedging put options. Furthermore, in Figure \ref{fig:AppputTotalW1}, Figure \ref{fig:putTotalW2}, and  Figure \ref{fig:putTotalW3}, we compare the distribution of the relative hedging error of $\modelT$ with the distributions of the relative hedging error of $\modelL$, BS model, and Bartlett model respectively.
...
\newpage
\subsubsection{Put Option Monthly Hedging Comparison}
In Table \ref{table:AppputTotalM}, we demonstrate the results on monthly hedging put options. Furthermore, in Figure \ref{fig:putTotalM1}, Figure \ref{fig:putTotalM2}, and  Figure \ref{fig:putTotalM3}, we compare the distribution of the relative hedging error of $\modelT$ with the distributions of the relative hedging error of $\modelL$, BS model, and Bartlett model respectively.
