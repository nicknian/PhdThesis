...
% % Furthermore, if we assume $\mu_{L}=\mu_{U}=0$ , we only have 4 parameters to be solved:
% % \[\{\sigma_{L},\sigma_{U}, \lambda_{L}, \lambda_{U}\}.\]
% % This fitting can be more robust and easier to be solved. In our thesis, we assume  $\mu_{L}=\mu_{U}=0$.
% \begin{itemize}
% \item Let $C(T,K;\sigma)$ and $C(T,K;\sigma)$ be the call and put option prices with the Black's implied volatility. 
% \item  Let $DC(T,K;\sigma)$ and $DC(T,K;\sigma)$ be the digital call and put option prices with the Black's implied volatility where a digital call pays on dollar if the underlying price exceeds the strike and a digital put pays the same amount  if the underlying is below the strike. 
% \item We have $\sigma_B$ given by the SABR apprximation formular \eqref{eq:SABRExpansion} for $K \in [K_{L},K_{U}]$. For simplicity, we wrote $\sigma_{B}(K)$ to denote the implied black's volatility given by the SABR formula \eqref{eq:SABRExpansion} for strike $K$.
% \end{itemize}
% Let $\mathcal{N}$ be the cumulative density function of standard normal distribution, we can have the following close-form formula:
% \[
% \int_{K_{U}}^{\infty}	 \lambda_{U} \;  q(x;\mu_{U},\sigma_{U}) dx=1-\lambda_{U} \mathcal{N}(\frac{\ln(K_{U})-\mu_{U}}{\sigma_{U}})
% \]
% \[
% 	\int_{0}^{K_{L}}	\lambda_{L}   q(x;\mu_{L},\sigma_{L}) dx=\lambda_{L} \mathcal{N}(\frac{\ln(K_{L})-\mu_{L}}{\sigma_{L}})
% \]
% % \[
% % \int_{K_{L}}^{K_{U}}	  p(x;T,S_t)  dx=	\frac{1}{D(t;T)} \left( \left. \frac{\partial C_{SABR}(T,K)}{\partial K} \right\vert_{K=K_{U}} -  \left.\frac{\partial C_{SABR}(T,K)}{\partial K} \right\vert_{K=K_{L}} \right)
% % \]
% \[
% \int_{K_{U}}^{\infty}	 x \lambda_{U} \;  q(x;\mu_{U},\sigma_{U}) dx=\lambda_{U} \mathcal{N}(\frac{\ln(K_{U})-\mu_{U}-\sigma_{U}^2}{\sigma_{U}})
% \]
% \[
% 	\int_{0}^{K_{L}}	x \lambda_{L}   q(x;\mu_{L},\sigma_{L}) dx=\lambda_{L} \mathcal{N}(\frac{\mu_{L}+\sigma_{L}^2-\ln(K_{L})}{\sigma_{L}})
% \]
% \[
% \begin{split}
% \int_{K_{L}}^{K_{U}}	 x p(x;T,S_t)  dx &=	\frac{1}{D(t;T)} \left( 
% 	C_{SABR}(K_{U},T) -K_{U}\left.\frac{\partial C_{SABR}(T,K)}{\partial K} \right\vert_{K=K_{U}}\right) \\
% 	&-
% 	\frac{1}{D(t;T)}
% 	\left( 
% 	C_{SABR}(K_{L},T) -K_{L}
% 	\left.\frac{\partial C_{SABR}(K,T)}{\partial K} \right\vert_{K=K_{L}} \right)
% \end{split}
% \]
We observe that:
\[
